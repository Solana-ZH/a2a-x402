...
This demo supports **Solana SPL token payments** (like USDC) in addition to EVM-based payments!

📖 **[Complete Solana Setup Guide →](SOLANA_PAYMENTS.md)**
...
```bash
# Set mock mode (no blockchain needed)
...
For real devnet transactions, see **[SOLANA_PAYMENTS.md](SOLANA_PAYMENTS.md)** for complete setup instructions.

### Switching Payment Networks

The merchant agent supports both **Solana** and **EVM** (Base Sepolia) payment networks. You can switch between them using the `PAYMENT_NETWORK` environment variable:

```bash
# Use Solana payments (default)
export PAYMENT_NETWORK=solana
# or omit the variable entirely

# Use EVM/Base Sepolia payments
export PAYMENT_NETWORK=evm
```

**Note:**

- Solana payments work with both mock and real mode (`USE_MOCK_FACILITATOR`)
- EVM payments currently only work with mock mode in this demo
- The `MultiChainMerchantExecutor` automatically routes payments to the correct facilitator based on the network
